...
const publicApplicationSchema = z.object({
  ...
  truck_size: z.string().max(80).optional().default(""),
...
async function organizationForKey(key) {
  const organization = await db("organizations")
    .where({ public_apply_key: key })
    .first();
  if (!organization)
    throw new ApiError(404, "This application link is no longer available");
  return organization;
}

async function applicationForOrg(orgId, id) {
  const application = await db("vendor_applications")
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      markets: markets.map((market) => {
        let vendor_details = {};
        try {
          vendor_details = JSON.parse(market.vendor_details || "{}");
        } catch {
          vendor_details = {};
        }
  ...
  asyncHandler(async (req, res) => {
    ...
    const application = await db.transaction(async (trx) => {
      ...
      const vendorValues = {
        ...
        booth_type: req.body.booth_type,
        instagram: req.body.instagram,
        tiktok: req.body.tiktok,
        facebook: req.body.facebook,
        stage: "Applied",
      };
      const vendorId = existingVendor
        ? existingVendor.id
        : await insertId(trx, "vendors", {
            ...vendorValues,
            org_id: organization.id,
          });
      if (existingVendor)
        await trx("vendors").where({ id: vendorId }).update(vendorValues);

      const vendorMarketKey = {
        org_id: organization.id,
        vendor_id: vendorId,
        market_id: market.id,
      };
      ...
      if (link)
        await trx("vendor_markets")
          .where({ id: link.id })
          .update({ stage_override: "Applied" });
      else
        await trx("vendor_markets").insert({
          ...vendorMarketKey,
          stage_override: "Applied",
        });
      ...
      const applicationId = await insertId(trx, "vendor_applications", {
        ...
        category: req.body.category,
        booth_type: req.body.booth_type,
        booth_size: req.body.booth_type === "truck" ? req.body.truck_size : "",
        power_needed: false,
        instagram: req.body.instagram,
        tiktok: req.body.tiktok,
        facebook: req.body.facebook,
      ...
      if (req.body.assets.length) {
        await trx("application_assets").insert(
          req.body.assets.map((asset) => ({
            ...
            size_bytes: Buffer.byteLength(asset.data),
            data: asset.data,
          })),
        );
      }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const asset of assets)
      byApplication.set(asset.application_id, [
        ...(byApplication.get(asset.application_id) || []),
        asset,
      ]);
    res.json({
      applications: applications.map((application) => ({
        ...application,
        assets: byApplication.get(application.id) || [],
      })),
    });
  }),
);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    await db.transaction(async (trx) => {
      ...
      if (req.body.status) {
        ...
        await trx("vendors")
          ...
          .update({ stage });
        const link = await trx("vendor_markets")
          .where({
    ...
    const updated = await db("vendor_applications")
      .where({ id: application.id })
      .first();
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const application = await applicationForOrg(req.user.org_id, req.params.id);
    const asset = await db("application_assets")
      .where({
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="${asset.file_name.replace(/[\\\"]+/g, "_")}"`,
    );
...
router.post(
  "/application-link/rotate",
  requireRole("owner"),
  asyncHandler(async (req, res) => {
    const key = publicKey();
    await db("organizations")
      .where({ id: req.user.org_id })
      .update({ public_apply_key: key });
    res.json({ key });
  }),
);
