...
export default function ApplicationsTab({ user, canWrite, notify }) {
  ...
  const applicationUrl = organization?.public_apply_key
    ? `${window.location.origin}/#/apply/${organization.public_apply_key}`
    : "";
  ...
  const copyLink = async () => {
    try {
      ...
      notify("Copy the link from the box below", "err");
    }
  };
  const rotateLink = async () => {
    if (
      !window.confirm(
        "Replace the current vendor application link? The old link will stop working.",
      )
    )
      return;
    setBusy("rotate");
  ...
  const update = async (application, status = application.status) => {
    ...
    try {
      if (status === "approved") {
        const result = await api.approveVendorApplication(application.id);
        notify(
          `Approved for ${new Date(`${String(result.event_date).slice(0, 10)}T12:00:00`).toLocaleDateString()} — payment email queued`,
        );
      } else {
        await api.updateApplication(application.id, {
          status,
          review_notes: notes[application.id] || null,
        });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        <div className="flex items-start gap-3">
          <div className="flex-1">
            ...
            <p style={{ color: C.sub }} className="text-[12px] mt-1">
              Share this with vendors. Contact details are required; social
              handles, insurance, and product or booth photos are optional and
              saved to your private CRM when provided.
            </p>
          </div>
          <button
            onClick={load}
        ...
        <div className="flex gap-2 mt-3">
          <input
            ...
            style={inp}
            className="flex-1 min-w-0 px-3 py-2 rounded-lg text-[12px] outline-none"
          />
          <button
            onClick={copyLink}
      ...
      <div className="flex gap-2 flex-wrap">
        {["all", ...STATUSES].map((status) => (
          <button
            ...
            style={{
              background: filter === status ? C.pine : C.card,
              color: filter === status ? "#fff" : C.sub,
              border: `1px solid ${filter === status ? C.pine : C.line}`,
            }}
      ...
      {filtered.length === 0 && (
        <p style={{ color: C.faint }} className="text-[13px] py-8 text-center">
          No applications in this view yet.
        </p>
      )}
      {filtered.map((application) => {
        const insurance = application.assets.find(
          (asset) => asset.kind === "insurance",
        );
        const photos = application.assets.filter(
          (asset) => asset.kind !== "insurance",
        );
        return (
          <article
          ...
          >
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p style={{ color: C.sub }} className="text-[12px] mt-0.5">
                  {application.market_name} · submitted{" "}
            ...
            <div className="grid sm:grid-cols-2 gap-2 mt-3 text-[12px]">
              ...
              >
                ...
                {application.website && (
                  <>
                    <b>Website:</b> {application.website}
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {insurance && (
                <button
                  onClick={() => openAsset(application, insurance)}
                ...
                >
                  <FileText size={14} /> Insurance <ExternalLink size={12} />
                </button>
              )}
              {photos.map((asset) => (
                <button
                  key={asset.id}
                  onClick={() => openAsset(application, asset)}
                  style={{ background: C.paper2, color: C.sub }}
                  className="px-2.5 py-1.5 rounded-lg text-[11.5px] font-bold flex items-center gap-1"
                >
                  <Image size={14} /> {asset.file_name}{" "}
              ...
              >
                ...
                <div className="flex gap-2 flex-wrap mt-2">
                  {STATUSES.map((status) => (
                    <button
                      key={status}
                      onClick={() => update(application, status)}
                      disabled={busy === String(application.id)}
                      style={
                        status === "approved"
                          ? { background: C.pine, color: "#fff" }
                          : status === "unapproved"
