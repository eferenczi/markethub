...
const PROVIDERS = [
  ...
  },
  {
    ...
    Icon: CreditCard,
    tone: C.honeyDeep,
    blurb: "Store your PayPal merchant credentials for vendor payment setup.",
    fields: [
      {
        key: "client_id",
        label: "Client ID",
      ...
      },
      {
        ...
        placeholder: "payments@yourmarket.com",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      },
      {
        key: "from_email",
        label: "From email",
        placeholder: "markets@yourdomain.com",
      },
    ],
  },
  {
...
function IntegrationCard({ meta, state, onSaved, onDeleted, notify }) {
  ...
  const [form, setForm] = useState(() => {
    const init = {};
    for (const fdef of meta.fields)
      init[fdef.key] = fdef.secret ? "" : state?.fields?.[fdef.key] || "";
    return init;
  });
    ...
    >
      <div className="flex items-center gap-3 mb-1">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p
              style={{ fontFamily: FD, fontWeight: 600 }}
              className="text-[16px]"
            >
              {meta.name}
            </p>
            {configured ? (
              <Pill bg={C.sageSoft} fg={C.pine}>
                Connected
              </Pill>
            ) : (
              <Pill bg={C.paper2} fg={C.sub}>
                Not connected
              </Pill>
            )}
          </div>
        ...
        {meta.fields.map((fdef) => (
          <label key={fdef.key} className="block">
            ...
            >
              {fdef.label}
              {fdef.required ? " *" : ""}
              {fdef.secret && configured && state?.fields?.[fdef.key]
                ? `  · saved ${state.fields[fdef.key]}`
                : ""}
            </span>
            <input
      ...
      <div className="flex items-center gap-2 mt-4">
        <button
          onClick={save}
          disabled={busy}
          style={{ background: C.pine, color: "#fff", opacity: busy ? 0.6 : 1 }}
          className="px-4 py-2 rounded-lg text-[13px] font-bold flex items-center gap-1.5"
        >
          {busy === "save" ? (
        ...
        {configured && (
          <button
            ...
            className="px-3 py-2 rounded-lg text-[13px] font-bold flex items-center gap-1.5"
          >
            {busy === "del" ? (
              <Loader2 size={14} className="animate-spin" />
            ) : (
...
function TeamSection({ user, notify }) {
  ...
  const invite = async () => {
    if (!inv.name || !inv.email)
      return notify("Name and email required", "err");
    setBusy(true);
    try {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <p style={{ fontFamily: FD, fontWeight: 600 }} className="text-[16px]">
          Team
        </p>
      </div>
          ...
          >
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-semibold truncate">
                {m.name}{" "}
                {!m.active && (
                  <span style={{ color: C.faint }} className="font-normal">
                    · invite pending
                  </span>
                )}
              </p>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <select
          ...
          style={inp}
          className="px-2 py-2 rounded-lg text-[13px] font-semibold outline-none"
        >
          <option value="manager">manager</option>
          <option value="staff">staff</option>
          <option value="vendor">vendor</option>
        ...
        <button
          ...
          disabled={busy}
          style={{ background: C.pine, color: "#fff", opacity: busy ? 0.6 : 1 }}
          className="px-4 py-2 rounded-lg text-[13px] font-bold flex items-center justify-center gap-1.5"
        >
          {busy ? (
            <Loader2 size={14} className="animate-spin" />
          ) : (
    ...
    >
      <div style={{ maxWidth: 820, margin: "0 auto" }} className="px-4 py-6">
        <button
          onClick={onClose}
          style={{ color: C.sub }}
          className="text-[13px] font-semibold flex items-center gap-1.5 mb-4"
        >
        ...
        <p style={{ color: C.sub }} className="text-[13.5px] mb-6">
          Connect your own payment and messaging accounts. Keys are encrypted
        ...
        {loadErr && (
          <div
            ...
            className="rounded-lg px-4 py-3 text-[13px] mb-4 flex items-center gap-2"
          >
            <AlertTriangle size={16} /> {loadErr}
          </div>
        )}
              ...
              <IntegrationCard
                ...
                onSaved={(integration) =>
                  setIntegrations((list) =>
                    list.map((i) => (i.provider === meta.id ? integration : i)),
                  )
                }
                onDeleted={() =>
                  setIntegrations((list) =>
        ...
        <p
          style={{ color: C.faint }}
          className="text-[11.5px] mt-6 leading-relaxed"
        >
          For real payment volume, Stripe recommends Stripe Connect (organizers
          authorize your platform instead of pasting a secret key). This
          settings model stores each org's own keys, encrypted.
        </p>
      </div>
