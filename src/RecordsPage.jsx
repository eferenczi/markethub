...
import {
  Building2, Store, Plus, Trash2, Loader2, Check, X, ArrowLeft, Pencil, CalendarDays, CreditCard, LayoutDashboard, Users, ClipboardList, MapPinned, Send, ReceiptText,
} from "lucide-react";
import { api } from "./api";
    ...
    <div style={{ background: C.paper2, minHeight: "100vh", fontFamily: FB, color: C.ink }} className="p-3 sm:p-5">
      <div style={{ maxWidth: 1260, margin: "0 auto", background: C.card, border: `1px solid ${C.line}` }} className="rounded-2xl overflow-hidden shadow-xl shadow-black/5 flex min-h-[760px]">
        <aside style={{ background: C.pineDeep, width: 230 }} className="hidden md:flex flex-col p-3 text-white">
          ...
          <div className="flex flex-col gap-1 flex-1">{[["dashboard", "Dashboard", LayoutDashboard], ["markets", "Markets", Building2], ["vendors", "Vendors", Users], ["applications", "Applications", ClipboardList], ["operations", "Event operations", CalendarDays], ["financials", "Financial reports", ReceiptText], ["campaigns", "Campaigns", Send]].map(([key, label, Icon]) => <button key={key} onClick={() => setTab(key)} style={{ background: tab === key ? "rgba(255,255,255,.13)" : "transparent", color: tab === key ? "#fff" : "rgba(255,255,255,.65)" }} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-left"><Icon size={17} /> {label}</button>)}</div>
