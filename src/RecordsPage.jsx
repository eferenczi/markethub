...
import {
  Building2, Store, Plus, Trash2, Loader2, Check, X, ArrowLeft, Pencil, CalendarDays, CreditCard, LayoutDashboard, Users, ClipboardList, MapPinned, Send, ReceiptText, Layers3,
} from "lucide-react";
import { api } from "./api";
    ...
    <div style={{ background: C.paper2, minHeight: "100vh", fontFamily: FB, color: C.ink }} className="p-3 sm:p-5">
      <div style={{ maxWidth: 1260, margin: "0 auto", background: C.card, border: `1px solid ${C.line}` }} className="rounded-2xl overflow-hidden shadow-xl shadow-black/5 flex min-h-[760px]">
        <aside style={{ background: C.pineDeep, width: 230 }} className="hidden md:flex flex-col p-3 text-white">
          ...
          <div className="flex flex-col gap-1 flex-1">{[["dashboard", "Dashboard", LayoutDashboard], ["markets", "Markets", Building2], ["vendors", "Vendors", Users], ["applications", "Applications", ClipboardList], ["operations", "Event operations", CalendarDays], ["templates", "Templates", Layers3], ["financials", "Financial reports", ReceiptText], ["campaigns", "Campaigns", Send]].map(([key, label, Icon]) => <button key={key} onClick={() => setTab(key)} style={{ background: tab === key ? "rgba(255,255,255,.13)" : "transparent", color: tab === key ? "#fff" : "rgba(255,255,255,.65)" }} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-left"><Icon size={17} /> {label}</button>)}</div>
        ...
        <main className="flex-1 min-w-0"><div style={{ borderBottom: `1px solid ${C.line}`, background: C.card }} className="px-5 sm:px-7 py-4"><div className="flex items-center gap-3"><div className="flex-1"><p style={{ color: C.faint }} className="text-[10px] font-bold uppercase tracking-wide">Organizer console</p><h1 style={{ fontFamily: FD }} className="text-[23px] font-semibold">{({ dashboard: "Dashboard", markets: "Markets & pricing", vendors: "Vendor CRM", applications: "Vendor applications", operations: "Event operations", templates: "Templates", financials: "Financial reports", campaigns: "Campaigns & newsletter" })[tab]}</h1></div>{onClose && <button onClick={onClose} style={{ color: C.sub }} className="text-[12px] font-semibold flex items-center gap-1"><ArrowLeft size={14} /> Back</button>}</div><div className="md:hidden flex gap-2 overflow-x-auto mt-3 pb-1">{[["dashboard", "Dashboard"], ["markets", "Markets"], ["vendors", "Vendors"], ["applications", "Applications"], ["operations", "Events"], ["templates", "Templates"], ["financials", "Financials"], ["campaigns", "Campaigns"]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} style={{ background: tab === key ? C.pine : C.paper2, color: tab === key ? "#fff" : C.sub }} className="px-3 py-1.5 rounded-full text-[11.5px] font-semibold whitespace-nowrap">{label}</button>)}</div></div><div className="p-5 sm:p-7 max-w-6xl">{tab === "dashboard" ? <DashboardTab go={setTab} notify={notify} /> : tab === "markets" ? <MarketsTab canWrite={canWrite} notify={notify} /> : tab === "vendors" ? <VendorsTab canWrite={canWrite} notify={notify} /> : tab === "applications" ? <ApplicationsTab user={user} canWrite={canWrite} notify={notify} /> : tab === "campaigns" ? <CampaignsTab canWrite={canWrite} notify={notify} /> : tab === "templates" ? <TemplatesPage canWrite={canWrite} notify={notify} /> : tab === "financials" ? <FinancialReports notify={notify} /> : <EventHub canWrite={canWrite} notify={notify} />}</div></main>
      </div>
      <Toast toast={toast} />
    </div>
  );
