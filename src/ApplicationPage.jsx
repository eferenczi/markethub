...
export default function ApplicationPage({ applicationKey }) {
  ...
  const submit = async () => {
    ...
    if (![form.instagram, form.tiktok, form.facebook].some(Boolean)) return setError("Please add at least one social media handle.");
    if (!insurance) return setError("Please upload your insurance document.");
    if (!photos.length) return setError("Please upload at least one booth or product photo.");
    setBusy(true);
    try {
  ...
  return <div style={{ background: C.paper, minHeight: "100vh", fontFamily: FB, color: C.ink }} className="py-8 px-4"><main style={{ maxWidth: 720, margin: "0 auto" }}><div className="text-center mb-7"><div style={{ background: C.pine }} className="w-11 h-11 rounded-xl inline-flex items-center justify-center mb-3"><Store size={22} color={C.honey} /></div><h1 style={{ fontFamily: FD }} className="text-[29px] font-semibold">Vendor application</h1><p style={{ color: C.sub }} className="text-[14px] mt-1">Apply to {info.organization.name}</p></div>
    <div style={{ background: C.card, border: `1px solid ${C.line}` }} className="rounded-2xl p-5 sm:p-7 flex flex-col gap-5">
      ...
      <section><h2 style={{ fontFamily: FD }} className="text-[19px] font-semibold mb-3">Business and contact</h2><div className="grid sm:grid-cols-2 gap-3"><input value={form.business_name} onChange={(e) => set("business_name", e.target.value)} placeholder="Business name *" style={inp} className="px-3 py-2.5 rounded-lg text-[14px] outline-none" /><input value={form.contact_name} onChange={(e) => set("contact_name", e.target.value)} placeholder="Your full name *" style={inp} className="px-3 py-2.5 rounded-lg text-[14px] outline-none" /><input value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="Phone *" style={inp} className="px-3 py-2.5 rounded-lg text-[14px] outline-none" /><input value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="Email *" type="email" style={inp} className="px-3 py-2.5 rounded-lg text-[14px] outline-none" /><input value={form.city} onChange={(e) => set("city", e.target.value)} placeholder="City" style={inp} className="px-3 py-2.5 rounded-lg text-[14px] outline-none" /><input value={form.category} onChange={(e) => set("category", e.target.value)} placeholder="Product category / cuisine" style={inp} className="px-3 py-2.5 rounded-lg text-[14px] outline-none" /></div></section>
